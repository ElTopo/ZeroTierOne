...
#      0. change DESTDIR?= to DESTDIR?=/opt
...
ifeq ($(origin CC),default)
        CC:=$(shell if [ -e /opt/bin/clang ]; then echo clang; else echo gcc; fi)
        CC:=$(shell if [ -e /opt/rh/devtoolset-8/root/usr/bin/gcc ]; then echo /opt/rh/devtoolset-8/root/usr/bin/gcc; else echo $(CC); fi)
endif
ifeq ($(origin CXX),default)
        CXX:=$(shell if [ -e /opt/bin/clang++ ]; then echo clang++; else echo g++; fi)
...
DEFS?=
LDLIBS?=
# lxl: set default DESTDIR to /opt
DESTDIR?=/opt
# lxl: set default DESTDIR to /opt
...
MINIUPNPC_IS_NEW_ENOUGH=$(shell grep -sqr '.*define.*MINIUPNPC_VERSION.*"2..*"' /opt/include/miniupnpc/miniupnpc.h && echo 1)
#MINIUPNPC_IS_NEW_ENOUGH=$(shell grep -sqr '.*define.*MINIUPNPC_VERSION.*"2.."' /opt/include/miniupnpc/miniupnpc.h && echo 1)
...
ifeq ($(wildcard /opt/include/natpmp.h),)
...
ifeq ($(ZT_CONTROLLER),1)
	override LDLIBS+=-L/opt/pgsql-10/lib/ -lpq ext/hiredis-0.14.1/lib/centos8/libhiredis.a ext/redis-plus-plus-1.1.1/install/centos8/lib/libredis++.a
	override DEFS+=-DZT_CONTROLLER_USE_LIBPQ
	override INCLUDES+=-I/opt/pgsql-10/include -Iext/hiredis-0.14.1/include/ -Iext/redis-plus-plus-1.1.1/install/centos8/include/sw/
endif

# ARM32 hell -- use conservative CFLAGS
ifeq ($(ZT_ARCHITECTURE),3)
	ifeq ($(shell if [ -e /opt/bin/dpkg ]; then dpkg --print-architecture; fi),armel)
...
# lxl: for target 'install' and 'uninstall', apply rules for /usr, /var
...
install:	FORCE
	...
	rm -f $(DESTDIR)/sbin/zerotier-one
	cp -f zerotier-one $(DESTDIR)/sbin/zerotier-one
	rm -f $(DESTDIR)/sbin/zerotier-cli
	rm -f $(DESTDIR)/sbin/zerotier-idtool
	ln -s zerotier-one $(DESTDIR)/sbin/zerotier-cli
	ln -s zerotier-one $(DESTDIR)/sbin/zerotier-idtool
	...
	ln -s $(DESTDIR)/sbin/zerotier-one $(DESTDIR)/var/lib/zerotier-one/zerotier-one
	ln -s $(DESTDIR)/sbin/zerotier-one $(DESTDIR)/var/lib/zerotier-one/zerotier-cli
	ln -s $(DESTDIR)/sbin/zerotier-one $(DESTDIR)/var/lib/zerotier-one/zerotier-idtool
	mkdir -p $(DESTDIR)/share/man/man8
	rm -f $(DESTDIR)/share/man/man8/zerotier-one.8.gz
	cat doc/zerotier-one.8 | gzip -9 >$(DESTDIR)/share/man/man8/zerotier-one.8.gz
	mkdir -p $(DESTDIR)/share/man/man1
	rm -f $(DESTDIR)/share/man/man1/zerotier-idtool.1.gz
	rm -f $(DESTDIR)/share/man/man1/zerotier-cli.1.gz
	cat doc/zerotier-cli.1 | gzip -9 >$(DESTDIR)/share/man/man1/zerotier-cli.1.gz
	cat doc/zerotier-idtool.1 | gzip -9 >$(DESTDIR)/share/man/man1/zerotier-idtool.1.gz
